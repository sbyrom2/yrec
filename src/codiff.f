C $$$$$$
C       SUBROUTINE CODIFF(DELTS,HRU,HV,IMIN,IMAX,LC,LCZ,M,RMID,COD,COD2)
...
C MHP 9/93
      COMMON/NOTRAN/LNOJ
      DIMENSION HRU(JSON),COD(JSON),  ! HV(JSON),LC(JSON),LCZ(JSON)
     *  RMID(JSON),COD2(JSON)
...
C  SVEL : RUN OF CONVECTIVE VELOCITIES.
C
C  OUTPUT VARIABLES
C
C       DECLARED
