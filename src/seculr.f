...
C FOR ANGULAR MOMENTUM PURPOSES.
     ...
     * HR(JSON),OMEGA(JSON),HL(JSON),ETA2(JSON),QIW(JSON),  ! LC(JSON)
...
C  SET UP FACTORS USED TO COMPUTE DIFFUSION VELOCITIES.
      CALL SETUPV(HD,HG,HL,HP,HR,HS1,HT,IMIN,IMAX,M,HRU,QWRMAX)  ! LCZ
      CLTOT = CLSUN*HL(M)
...
C MHP 06/02 added timestep to call
      CALL VCIRC(HR,HRU,IMIN,IMAX,IT,LCZ,M,OMEGA,LDO,QWLNR,  ! HJM
...
C G Somers 6/14, SET LIMIX = .FALSE. SO THE CORRECT GRADS ARE USED.
         LIMIX = .FALSE.
         CALL MIXCZ(HCOMP,HS2,LCZ,M)  ! HS1,HR,HP,HD,HG,LIMIX
C G Somers END
         ENDIF
...
C          CALL CODIFF(DELTS,HRU,HV,IMIN,IMAX,LC,LCZ,M,RMID,COD,COD2)
...
C  EQUALLY SPACED GRID POINTS.
            CALL TRIDIA(NTOT,EI,DJ,SUMDJ)  ! EJ
...
C SOLVE FOR OMEGA AND ITS DERIVATIVES IN A BAND MATRIX
            CALL DADCOEFT(DR,DT,EI,EW,NTOT,WIND1,WIND2,DJ,  ! EJ
...
C         WRITE(*,*)LSOLID,OMEGA(1),OMEGA(M)
     ...
     *         M,DT,ETA2,HI,HJM,IREDO,LOK,LREDO,OMEGA,QIW,R0,  ! LPRT
     *         WSAV,ID,IDM,LOKAD)
