C $$$$$$
      SUBROUTINE VCIRC(HR,HRU,IMIN,IMAX,IT,LCZ,M,OMEGA,LDO,  ! HJM
...
C TERMS
     ...
     *                QQQCOD(JSON)
      DIMENSION HR(JSON),HRU(JSON),LCZ(JSON),OMEGA(JSON),  ! HJM(JSON)
...
C  TREATMENT OF FIRST INTERFACE.
      IF(IMIN.LT.3)THEN
         VISCM(2)=EXP(LOG(VISC(1))*FACI(1,2)+LOG(VISC(2))*FACI(2,2)+
     *            LOG(VISC(3))*FACI(3,2)+LOG(VISC(4))*FACI(4,2))
         THDIFM(2)=EXP(LOG(THDIF(1))*FACI(1,2)+LOG(THDIF(2))*FACI(2,2)
     *            +LOG(THDIF(3))*FACI(3,2)+LOG(THDIF(4))*FACI(4,2))
         I0=3
      ELSE
         I0=IMIN
      ENDIF
C  TREATMENT OF LAST INTERFACE.
      IF(IMAX.EQ.M)THEN
         VISCM(M)=EXP(LOG(VISC(M-3))*FACI(1,M)+LOG(VISC(M-2))*FACI(2,M)
     *            +LOG(VISC(M-1))*FACI(3,M)+LOG(VISC(M))*FACI(4,M))
         THDIFM(M)=EXP(LOG(THDIF(M-3))*FACI(1,M)+LOG(THDIF(M-2))*
     *            FACI(2,M)+LOG(THDIF(M-1))*FACI(3,M)+
     *            LOG(THDIF(M))*FACI(4,M))
         I1=M-1
      ELSE
         I1=IMAX
      ENDIF
C  GENERAL CASE.
      DO 20 I = I0,I1
         VISCM(I)=EXP(LOG(VISC(I-2))*FACI(1,I)+LOG(VISC(I-1))
     *            *FACI(2,I)+LOG(VISC(I))*FACI(3,I)+LOG(VISC(I+1))
     *            *FACI(4,I))
         THDIFM(I)=EXP(LOG(THDIF(I-2))*FACI(1,I)+LOG(THDIF(I-1))
     *            *FACI(2,I)+LOG(THDIF(I))*FACI(3,I)+LOG(THDIF(I+1))
     *            *FACI(4,I))
   20 CONTINUE
C USE LINEAR INTERPOLATION FOR OMEGA AND MU.
      DO I = 2,M
         AMUM(I) = 0.5D0*(HAMU(I)+HAMU(I-1))
...
C            TKHS = SQRT(VISCM(I)*CC/THDIFM(I))
C            QMU = (HAMU(I)-HAMU(I-1))/(HRU(I)-HRU(I-1))
C            VMU(I) = ABS(FMU*FACT1(I)*FACT4(I)*AMUM(I)*QMU/TKHS)
C            VMU2(I)=FMU*HGM(I)*ABS(QMU)/AMUM(I)/WM(I)**2
C   32    CONTINUE
...
C            DR = HRU(I) - HRU(I-1)
C            VMU(I)=FMU*HGM(I)*ABS(HAMU(I)-HAMU(I-1))/DR/AMUM(I)
C     *             /WM(I)**2
C   33    CONTINUE
...
C            VMU(I)=FMU*RM(I)*DDEL*ABS(HAMU(I)-HAMU(I-1))/DR/AMUM(I)
C 34         CONTINUE
C      ENDIF
...
C MU GRADIENTS.
         ...
         DR = HRU(I)-HRU(I-1)
         IF(IGSF.EQ.0)THEN
            QWRMX=2.0D0*SQRT(HGM(I)*ABS(HAMU(I)-HAMU(I-1))
     *            /DR/AMUM(I))
            IF(ABS(QWLNR(I)).LT.QWRMX)THEN
               VGSF(I) = 0.0D0
               GOTO 40
...
C VELOCITIES ARE LATER MULTIPLIED BY R (THE "LENGTH SCALE").
            VGSF(I) = FGSFK(I)*FX*THDIFM(I)*WM(I)**2/RMID
         ELSE
            VGSF(I)=0.25D0*FGSFK(I)*THDIFM(I)*DLNWDR*WM(I)**2/RMID
         ENDIF
...
C MHP 8/93 STABILITY CONDITION ADDED, NEGLECTING THE EFFECTS OF
C MU GRADIENTS.
C            QWRMX = 2.0D0*SQRT(VISCM(I)/THDIFM(I))*QWRMAX(I)
...
C  VALUE AND COMPUTE (LARGE) DYNAMICAL SHEAR VELOCITY.
         IF(ABS(QWLNR(I)).GT.QWRMAX(I))THEN
            QWR = ABS(QWLNR(I))
            VSS(I)=8.0D0/4.5D1*THDIFM(I)*(QWR/QWRMAX(I))**2/RM(I)
...
C THE VELOCITY ESTIMATE HERE IS FROM ZAHN 1991.
         QWRMX = SQRT(VISCM(I)/THDIFM(I)*1.25D-1*RCRIT)*QWRMAX(I)
         IF(ABS(QWLNR(I)).GT.QWRMX) THEN
C  UNSTABLE; CHECK FOR MU GRADIENTS.
            IF(ABS((HAMU(I)-HAMU(I-1))/AMUM(I)).LT.1.0D-10) THEN
               QWRMX2 = 0.0D0
...
C      FACT = (RHOM/PM)*QTMU*DMU/AMUM/DP*HGM**2
               QWRMX2 = 2.0D0*SQRT(MAX(1.0D-20,FACT5(I)*
     *                  ABS((HAMU(I)-HAMU(I-1))/AMUM(I))))
