...
C                           G Somers, 3/17                         C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE GETTAU(HCOMP,HR,HP,HD,HS1,HT,FP,FT,TEFFL,HSTOT,BL,M,  ! HG
     *                  LC,RBCZ)
     ...
     * LOCONS(JSON),SO(JSON),SDEL(3,JSON),SFXION(3,JSON),SVEL(JSON)
      DIMENSION HCOMP(15,JSON),HR(JSON),HP(JSON),HD(JSON),HS1(JSON),  ! HG(JSON)
...
C CALL TAUINT
      IF(LNEWTCZ)THEN
         CALL TAUINTNEW(HSF,LCF,HRF,HPF,HDF,HGF,MM,M,HVF,  ! HCOMPF,HS2
     *                  DELF1,DELF2,RBCZ)  ! HSTOT
      ELSE
         CALL TAUINT(HSF,LCF,HRF,HPF,HDF,HGF,MM,HVF,  ! HCOMPF,HS2
     *               DELF1,DELF2)  ! HSTOT
      ENDIF
