...
C STARIN also calls RSCALE to perform rescaling if requested
       CALL STARIN(BL,CFENV,DAGE,DDAGE,DELTS,DELTSH,ETA2,  ! DELTS0
     *    FP,FT,FTRI,HCOMP,HD,HI,HJM,HKEROT,HL,HP,HR,HS,HS1,HS2,
     *    HSTOT,HT,IKUT,ISTORE,JENV,LARGE,LC,LNEW,M,MODEL,  ! JCORE
...
C SKIP CALCULATIONS FOR LOW TEMPERATURES.
     ...
     *TL,X,Y,XHE3,XC12,XC13,XN14,XO16,  ! PDT,PDP,Z,XN15,XO17
     *XO18,XH2,IU,HR1,HR2,HR3,HR4,HR5,HR6,HR7,  ! XLI6,XLI7,XBE9
...
C JVS 04/14 Added Teffl to passed variables
       CALL HTIMER(DELTS,DELTSH,M,HD,HL,HS1,HS2,HT,HCOMP,JCORE,  ! LC
     *               JXMID,TLUMX,DAGE,DDAGE,NK,HP,HR,OMEGA,  ! QDT,QDP
     *               DWMAX,JXBEG,TEFFL)
...
C STORE COMPOSITION MATRIX AT THE BEGINNING OF THE TIMESTEP.
   ...
   33          CONTINUE
     ...
     *                     HT,ITLVL,LC,M,DDAGE,JCORE,JENV,  ! QDT,QDP
     *                     MXZONE,MXZON0,TEFFL)
             DDAGE = DELTS/CSECYR
...
C rezone new model, except rezoning not performed for He flash calculations
     ...
     *              HJM,HKEROT,SJTOT,SKEROT,FP,FT,
     *              HG,QIW,TEFFL)  ! BL,DELTS,SMASS
C STORE NEW CZ BASE
               JCZ = JENV
            ELSE
...
C CALL TO CRRECT - ADDED ITERATION LEVEL
     ...
     *           LMIX,LOCOND,QDT,QDP,FP,FT,ETA2,OMEGA,R0,ITDONE,  ! MODEL
     *           HG,HI,HJM, ITLVL,LCZ,MXZONE,  ! MRZONE,NRZONE,NZONE
     *           QIW,HKEROT,HKEROTO)
...
C G Somers 6/14, SET LIMIX = .TRUE. SO THE CORRECT GRADS ARE USED.
      LIMIX = .TRUE.
      CALL MIXCZ(HCOMP,HS2,LC,M)  ! HS1,HR,HP,HD,HG,LIMIX
C G Somers END

C MHP 9/94 STORE TOTAL AGE IN SAGE
            SAGE = DAGE
C JVS 02/12 STITCH TOGETHER CONVERGED INTERIOR, ENVELOPE, AND ATMOSPHERE
C            CALL STITCH(HCOMP,HR,HP,HD,HS,HT,FP,FT,TEFFL,HSTOT,BL,M,  ! HG
...
C JENV0 ADDED TO SR CALL.
     ...
     *              HS,HS1,HS2,HSTOT,HT,LC,LJDOT,M,SMASS,  ! MODEL,SJTOT
...
C FIND BURNING RATES AT THE END OF THE TIME STEP.
               ...
               CALL LIBURN(DELTS,HCOMP,HR,HS1,HS2,HT,JENV1,JENV0,M)  ! HD
            ENDIF
         ENDIF
...
C INCLUDED
     ...
     *                     HSTOT,HT,LC,ETA2,HI,HJM,QIW,R0,  ! HG
...
C      *        CFENV,FTRI,HSTOT,OMEGA,RLL,ICONV,NK,NN)
         CALL WRTMONTE(HCOMP,HD,HL,HP,HR,HS,HT,LC,M,DAGE,  ! MODEL
     *        DDAGE,SMASS,TEFFL,BL,JCORE,JENV,  ! GL,LSHELL,JXBEG,JXMID,JXEND
