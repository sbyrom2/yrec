...
C 11/91 JENV0 ADDED TO CALL.
     ...
     *             HS1,HS2,HSTOT,HT,LC,LJDOT,M,SMASS,  ! MODEL,SJTOT
...
C  KINEMATIC VISCOSITIES,THERMOMETRIC DIFFUSIVITY, AND HEAT CAPACITY.
      CALL PHYSIC(FP,FT,HCOMP,HD,HG,HL,HP,HR,HS,HT,LC,M,TEFFL)  ! LCZ
C 8/17 DETERMINE TAUCZ AND PPHOT
      CALL GETTAU(HCOMP,HR,HP,HD,HS1,HT,FP,FT,TEFFL,  ! HG
     *            HSTOT,BL,M,LC,ENVR)
...
C JVS
     ...
     *            HSTOT,HT,LC,LFIRST,M,ETA2M,HDM,HGM,HICZ,HIM,  ! LCZ
...
C G Somers 6/14, SET LIMIX = .FALSE. SO THE CORRECT GRADS ARE USED.
            LIMIX = .FALSE.
          CALL MIXCZ(HCOMP,HS2,LCZM,M)  ! HS1,HRM,HPM,HDM,HGM,LIMIX
C G Somers END
...
C FOR THE BUR-ST MIXING ROUTINES
         CALL SECULR(DT,HDM,HGM,HIM,HLM,HPM,HRM,HS,HS1,HS2,  ! DELTS
     *               HTM,HJMSAV,LCZM,M,OMEGAM,  ! LC,MODEL
     *               HJM,ETA2M,QIWM,R0M,HCOMP,IMIN,IMAX,BL,  ! LFIRST
     *               HSTOT,SMASS,TEFFL,LREDO,IREDO,  ! SJTOT
...
C COMPUTE BURNING.
            CALL LIBURN(DT,HCOMP,HRM,HS1,HS2,HTM,JENV1,JENV0,M)  ! HDM
            JENV0 = JENV1
            PSCAL0 = PSCAL1
            DO 155 I = 1,M
               RLI60(I) = RLI6(I)
  ...
  155       CONTINUE
         ELSE
C COMPUTE BURNING.
            CALL LIBURN2(DT,HCOMP,HRM,HS1,HS2,HTM,JENV1,JENV0,M)  ! HDM
         ENDIF
C  ENSURE THAT CONVECTIVE REGIONS ARE FULLY MIXED.
C  JVS 0212       CALL MIXCZ(HCOMP,HS2,LCZM,M)
C KC 2025-05-30 addressed warning messages from Makefile.legacy
